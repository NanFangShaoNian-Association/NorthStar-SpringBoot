...
//        log.info("成功");
        ...
        if (null != claims) {
            ...
            if (AccountRole.USER.getName().equals(role)) {
                ...
                String redisToken = String.valueOf(cacheClient.queryWithPassThrough(LOGIN_USER_KEY, user.getPhone(), RedisData.class,
                        s -> null, LOGIN_USER_TTL, TimeUnit.SECONDS).getData());
            ...
            }else if (AccountRole.MERCHANT.getName().equals(role)) {
                //todo:商家验证逻辑
            }else if (AccountRole.PLATFORM_ADMINISTRATOR.getName().equals(role)) {
                //todo:平台管理员验证逻辑
            }
        }
        return false;
